...
const pruebaSchema = require('./prueba.json');

describe('Test', () => {
  // it('should generate test data with differents variations based on required property', done => {
  // generate(userSchema, (err, res) => {
  //   if (err) {
  //     throw err;
  //   }
  //   res.forEach(links => {
  //     if (links.link.schema && links.link.schema.required) {
  //       const required = links.link.schema.required;
  ...
  it('should generate test data with negative type tests for simple primitive data types', done => {
    // console.log('¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡¡', pruebaSchema);
    $RefParser
      .dereference(pruebaSchema)
      .then(schema => {
        ...
        // console.log('res-> ', schema);
        let result = generate(schema.paths['/devices/commerce/link'].post.parameters[0].schema);
        console.log('res-> ', util.inspect(result));
      })
      .catch(err => {
        console.error(err);
      });
    // generate(pruebaSchema.paths['/commerces/stores'].post.parameters[0].schema, (err, res) => {
    //   res.forEach(links => {
    //     if (links.link.schema && links.link.schema.required) {
    //       const required = links.link.schema.required;
    //       const data = links.data;
    ...
    //           .some(d => {
    //             const types = jstdgen.getNegativeTypes(userSchema.properties[req]);
